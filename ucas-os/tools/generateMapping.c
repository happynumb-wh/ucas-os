...
name_table map_name[7] = { 
    {.sname = "buildshell", .dname = "shell"},
    {.sname = "imgargc", .dname = "argc"}
    
};

char *handle_name(char * ret)
{
    // for (int i = 0; i < 2; i++)
    // {
    //     if (!strcmp(ret, map_name[i].sname))
    //         return map_name[i].dname;
    // }
    return ret;
}
...
char* simplify(Records* self, char* varname)
{
    ...
    simplify_filter(ret, varname, "_elf_");
    return ret;
}
