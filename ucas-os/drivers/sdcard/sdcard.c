...
uint8_t sd_read_sector_bio(struct buf *a, uint32_t count)
{
	...
	if (count == 1) {
		flag = 0;
		sd_send_cmd(SD_CMD17, a->sectorno, 0);
	} else {
		flag = 1;
		sd_send_cmd(SD_CMD18, a->sectorno, 0);
	}
	/*!< Check if the SD acknowledged the read block command: R1 response (0x00: no errors) */
	uint32_t result1;
	if ((result1 = sd_get_response()) != 0x00) {
...
uint8_t sd_write_sector_bio(struct buf *a, uint32_t count)
{
	...
	if (count == 1) {
		frame[1] = SD_START_DATA_SINGLE_BLOCK_WRITE;
		sd_send_cmd(SD_CMD24, a->sectorno, 0);
	} else {
		frame[1] = SD_START_DATA_MULTIPLE_BLOCK_WRITE;
		sd_send_cmd(SD_ACMD23, count, 0);
		sd_get_response();
		sd_end_cmd();
		sd_send_cmd(SD_CMD25, a->sectorno, 0);
	}
	/*!< Check if the SD acknowledged the write block command: R1 response (0x00: no errors) */
	uint32_t result1;
	if ((result1 = sd_get_response()) != 0x00) {
